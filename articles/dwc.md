...
$$ \\text{time difference}=\\text{time}_{\\text{Left}}-\\text{time}_{\\text{Right}}$$ 

$$ X=A \\cdot \\left(\\text{time difference}_{\\text{right}} + B $$ where the A and B are constants that were measured during the calibration. More specifically, A is the slope and B is the offset.
